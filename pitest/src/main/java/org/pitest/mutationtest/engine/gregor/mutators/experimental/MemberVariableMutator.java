    ...
    private final class MemberVariableVisitor extends MethodVisitor {
        ...
        public void visitFieldInsn(final int opcode, final String owner, final String name, final String desc) {
            if ((Opcodes.PUTFIELD == opcode) && shouldMutate(name)) {
                ...
                // pop the number to be assigned (can be one-word or two-word), then pop the
                // object loaded by ALOAD
        ...
        private boolean shouldMutate(final String fieldName) {
            final MutationIdentifier mutationId = this.context.registerMutation(MemberVariableMutator.this,
                    "Removed assignment to member variable " + fieldName);
