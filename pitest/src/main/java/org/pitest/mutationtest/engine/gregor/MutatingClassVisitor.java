...
class MutatingClassVisitor extends ClassVisitor {
    ...
    public MethodVisitor visitMethod(final int access, final String methodName, final String methodDescriptor,
        ...
        final MethodMutationContext methodContext = new MethodMutationContext(this.context,
        ...
        final MethodVisitor methodVisitor = this.cv.visitMethod(access, methodName, methodDescriptor, signature,
                exceptions);

        final MethodInfo info = new MethodInfo().withOwner(this.context.getClassInfo()).withAccess(access)
 ...
 */
    private MethodVisitor visitMethodForMutation(MethodMutationContext methodContext, final MethodInfo methodInfo,
        ...
        return new InstructionTrackingMethodVisitor(
                wrapWithDecorators(methodContext, wrapWithFilters(methodContext, next)), methodContext);
    }

    private static MethodVisitor wrapWithDecorators(MethodMutationContext methodContext, final MethodVisitor mv) {
        return wrapWithBlockTracker(methodContext, wrapWithLineTracker(methodContext, mv));
    }

    private static MethodVisitor wrapWithBlockTracker(MethodMutationContext methodContext, final MethodVisitor mv) {
    ...
    private static MethodVisitor wrapWithStringSwitchFilter(MethodMutationContext methodContext,
            final MethodVisitor wrappedMethodVisitor) {
        return new AvoidStringSwitchedMethodAdapter(methodContext, wrappedMethodVisitor);
    }

    private static MethodVisitor wrapWithAssertFilter(MethodMutationContext methodContext,
            final MethodVisitor wrappedMethodVisitor) {
